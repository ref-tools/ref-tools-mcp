...
async function doSearch(query: string) {
  const url = getRefUrl() + "/search_documentation?query=" + encodeURIComponent(query) + (moduleNames ? "&moduleNames=" + moduleNames?.join(",") : "");
  console.error("[search]", url);

  if (!process.env.REF_ALPHA) {
    return {
      content: [{ type: "text", text: "Ref is not correctly configured. Reach out to hello@ref.tools for help." }],
    };
  }
...
async function doSearchWeb(query: string) {
  try {
    ...
    const formattedResults = data.docs.map((result: any) => {
