...
export const SEARCH_GRAPH_DESCRIPTION =
  "Run a Cypher query on the code graph and return matching chunks. Supported (subset): CREATE, MATCH (node-only or single hop), labels (:Label) and inline property filters, WHERE with =, !=, <, <=, >, >= and AND/OR/NOT, RETURN variables and properties, count(*), count(var), collect(var), AS aliases, DISTINCT, ORDER BY, LIMIT. Labels: Chunk, Code, File. Relationships: REFERENCES, CONTAINS. Introspection: CALL db.labels(). Note: chunks are extracted from returned node values. If you return only scalar properties (e.g., d.filePath), the tool will resolve filePath strings to their file chunks; returning node variables (e.g., RETURN d) is still recommended. Here is an example query to find references: MATCH (f:File:Chunk)-[:CONTAINS]->(u:Chunk)-[:REFERENCES]->(d:Chunk) WHERE d.filePath ENDS WITH '/searchdb.ts' RETURN DISTINCT f ORDER BY f.filePath"
