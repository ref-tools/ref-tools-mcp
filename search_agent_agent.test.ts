...
describe.skip('SearchAgent LLM agent fallback streaming', () => {
  it(
    'streams tool calls and returns markdown without OpenAI key (answer mode)',
    { timeout: 10000 },
    async () => {
      const dir = tmpDir()
      ...
      const res = await runAgentWithStreaming(
        agent,
        'alpha feature',
        (e) => events.push(e),
        'answer',
  ...
  it(
    'findContext mode selects chunks and emits finalize step without OpenAI key',
    { timeout: 10000 },
    async () => {
      const dir = tmpDir()
      ...
      const hasFinalize = events.some(
        (e) =>
          e.type === 'tool_call' && e.name === 'finalize_context' && (e as any).input?.finalize,
      )
