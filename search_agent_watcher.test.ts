...
async function eventually(assertFn: () => Promise<void> | void, timeoutMs = 4000, intervalMs = 100) {
...
describe('SearchAgent watcher add/remove', () => {
  it('indexes newly added files and makes them searchable', async () => {
    const dir = tmpDir()
    // initial repo with one file
    const f1 = path.join(dir, 'a.ts')
    fs.writeFileSync(f1, `export function alpha(){ return 1 } // alpha\n`)

    const agent = new SearchAgent(dir, { languages: ['typescript'], watch: true, pollIntervalMs: 100 })
    await agent.ingest()
  ...
  it('removes deleted files from the index so they no longer appear in search', async () => {
    const dir = tmpDir()
